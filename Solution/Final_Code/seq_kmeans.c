...

{
   ...
   do {
        ...
        /* parallel region: each thread works on a subset of objects */
        #pragma omp parallel private(i, j, index)
        {
            ...
            for (i=0; i<numObjs; i++) {
                ...
                if (membership[i] != index) localDelta += 1.0;

                /* assign the membership to object i */
                membership[i] = index;

                /* update local cluster center : sum of objects located within */
                localClusterSize[index]++;
                for (j=0; j<numCoords; j++)
                    localClusters[index][j] += objects[i][j];
            }

            /* reduction step (merge thread-local partial results) */
            #pragma omp atomic
            delta += localDelta;
        ...
        #pragma omp parallel for private(j) schedule(dynamic, 25)
        for (i=0; i<numClusters; i++) {
            for (j=0; j<numCoords; j++) {
